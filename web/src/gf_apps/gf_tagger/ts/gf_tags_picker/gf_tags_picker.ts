...
export async function init(p_log_fun) {
    ...
    $(container).find("#flows").css("display", "none");
    $(container).find("#flows_experimental").css("display", "none");

    //------------------

    $(container).find("#expand_btn").click(()=>{
        ...
        if (visible_bool) {
            ...
            // show
            $(container).find("#flows").css("display", "block");
            $(container).find("#flows_experimental").css("display", "block");
            visible_bool = true;
        }
    });
    ...
    for (const flow_map of all_flows_lst ) {
        ...
        $(container).find(`#${target_container_id_str}`).append(`
            <div class="flow_info">
