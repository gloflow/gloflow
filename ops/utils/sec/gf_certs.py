...
#---------------------------------------------------
def generate__cert_root(p_output_files_base_str,
	...
	# # ALREADY_EXISTS
	# if os.path.isfile(p_privkey_output__file_path_str) and \
	#     os.path.isfile(p_cert_output_file_path_str):
	#     print("CERT_ROOT ALREADY EXISTS")
	#     print("privkey - %s"%(os.path.abspath(p_privkey_output__file_path_str)))
	#     print("cert    - %s"%(os.path.abspath(p_cert_output_file_path_str)))
	#
	#     if gf_core_cli.confirm("recreate cert_root?"):
	#         # archive existing cert by renaming it, and allow for new cert to get created
	#         archive__old_cert(p_privkey_output__file_path_str,
	#             p_cert_output_file_path_str)
	#     else:
	#         return
	...
	c_lst = [
		# "-initca" - initialise new CA
		"cfssl gencert -initca %s"%(p_config__file_path_str),
		"|",
...
#-------------------------------------------------------------
def generate__cert_intermediate():

	True

#-------------------------------------------------------------
def generate__cert_leaf():
...
def archive_if_exists(p_files_base_str, p_sudo_bool = False):
	...
	stdout_str, _, return_code = gf_core_cli.run_cmd("%s ls -1 %s | grep '^%s'"%(sudo_str, dir_str, file_base_str), p_env_map = None, p_print_output_bool = False)
