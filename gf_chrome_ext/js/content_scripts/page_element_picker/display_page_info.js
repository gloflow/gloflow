...
function display_page_info(p_page_images_infos_lst,
	...
	check_images_exist_in_system(p_page_images_infos_lst,
							p_log_fun);
	...
	function create_close_btn() {
		...
		$(close_btn_element).css('background-image',
	...
	function view_image(p_image_map) {
		...
		$(container).find('#collection').append(image_in_page_element);
			...
			$(img).load(() => {
				const img_id_str = 'id_'+hash_code(full_img_src_str,
												p_log_fun);
	...
	function view_video(p_video_map) {
		...
		init_video_hud(video_in_page_element,
					p_video_map,
					p_log_fun);
...
//---------------------------------------------------
function init_video_hud(p_video_in_page_element,
				p_video_info_map,
				p_log_fun) {
	...
	const hud = $(
		...
		'</div>');
	const icons_chrome_ext_url_str = 'url('+chrome.extension.getURL('assets/icons.png')+')';
	$(hud).find('.icon').css('background-image',
							icons_chrome_ext_url_str);
...
//---------------------------------------------------
function view_gif_info(p_full_img_src_str,
					p_host_str,
					p_log_fun) {
