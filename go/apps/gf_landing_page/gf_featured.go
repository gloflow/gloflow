...
func get_featured_posts(p_max_random_cursor_position_int int, //500
	...
	featured_posts_lst := posts_to_featured(posts_lst,
										p_log_fun)
	return featured_posts_lst,nil
}
//------------------------------------------
func posts_to_featured(p_posts_lst []*gf_publisher_lib.Post,
					p_log_fun func(string,string)) []*Featured_post {
	p_log_fun("FUN_ENTER","gf_featured.posts_to_featured()")

	featured_posts_lst := []*Featured_post{}
	for _,post := range p_posts_lst {
		featured := post_to_featured(post,
								p_log_fun)
	...
	for _,featured := range featured_posts_lst {
		...
		if featured.Image_url_str == "" || featured.Image_url_str == "error" {
			err_msg_str := fmt.Sprintf("post with title [%s] has a image_src that is [%s]",
									featured.Title_str,
									featured.Image_url_str)
...
//------------------------------------------
func post_to_featured(p_post *gf_publisher_lib.Post,
				p_log_fun func(string,string)) *Featured_post {
