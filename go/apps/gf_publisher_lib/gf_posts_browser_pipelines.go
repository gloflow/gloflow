...
func Get_posts_page(p_page_index_int int,
	...
	page_lst,err              := DB__get_posts_page(cursor_start_position_int,
...
func Render_initial_pages(p_response_format_str string,
	...
	for i:=0;i<p_initial_pages_num_int;i++ {
		...
		p_log_fun("INFO",fmt.Sprintf(">>>>>>> start_position_int - %d - %d",
								start_position_int,
								p_page_size_int))

		//initial page might be larger then subsequent pages, that are requested 
		//dynamically by the front-end
		page_lst,err := DB__get_posts_page(start_position_int,
								p_page_size_int,
								p_mongodb_coll,
								p_log_fun)
		if err != nil {
	...
	err := posts_browser__render_template(posts_pages_lst,
									p_tmpl,
									p_page_size_int,
									p_resp,
									p_log_fun)
