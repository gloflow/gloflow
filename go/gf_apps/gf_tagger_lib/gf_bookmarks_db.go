...
func db__bookmark__create(p_bookmark *GFbookmark,
	...
	gfErr := gf_core.MongoInsert(p_bookmark,
		coll_name_str,
...
func db__bookmark__update_screenshot(p_bookmark_id_str gf_core.GF_ID,
	...
	coll := pRuntimeSys.Mongo_db.Collection("gf_bookmarks")
	...
	if err != nil {
		gfErr := gf_core.MongoHandleError("failed to update DB ",
...
func db__bookmark__get_all(p_user_id_str gf_core.GF_ID,
	...
	db_cursor, gfErr := gf_core.MongoFind(bson.M{
		...
		pRuntimeSys.Mongo_db.Collection("gf_bookmarks"),
		p_ctx,
		pRuntimeSys)
	if gfErr != nil {
		return nil, gfErr
	...
	var bookmarks_lst []*GFbookmark
	err := db_cursor.All(p_ctx, &bookmarks_lst)
	if err != nil {
		gfErr := gf_core.MongoHandleError("failed to get DB results of query to get all Bookmarks",
			"mongodb_cursor_all",
			map[string]interface{}{
				"user_id_str": p_user_id_str,
			},
